      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2">
            <span className="chip-signal">{lc(play.badge, locale)}</span>
            <span
              className="chip"
              style={{
                color: EFFORT_TONE[play.effort],
                borderColor: "rgba(255,255,255,0.1)",
              }}
            >
              {t("play.difficulty").replace("{x}", t(EFFORT_KEY[play.effort]))}
            </span>
          </div>
          <h3 className="mt-4 text-[26px] headline text-ink-50">
            {lc(play.title, locale)}
          </h3>
        </div>
        <div className="text-right">
          <div className="eyebrow">{t("play.bestApr")}</div>
          <div className="mt-1 text-[34px] font-medium tracking-tightest text-ink-50 mono-num">
            {lc(play.apr, locale)}
          </div>
          <div className="text-[11px] text-ink-400">{t("play.aprNote")}</div>
        </div>
      </div>

      <p className="mt-6 text-[14px] text-ink-200 leading-relaxed max-w-3xl">
        <span className="text-ink-400">{t("play.whyLabel")}</span>
        {lc(play.why, locale)}
      </p>

      <div className="mt-7 grid md:grid-cols-[1.4fr_1fr] gap-5">
        <div>
          <div className="eyebrow mb-3">{t("play.steps")}</div>
        ...
        <div className="space-y-3">
          <div className="rounded-xl border border-signal/30 bg-signal/5 p-4">
            <div className="eyebrow text-signal mb-1.5">{t("play.upside")}</div>
            <div
              className="text-[13px] text-ink-100 leading-relaxed"
              dangerouslySetInnerHTML={{
                __html: lc(play.reward, locale).replace(
                  /\*\*(.*?)\*\*/g,
          ...
          <div className="rounded-xl border border-warn/30 bg-warn/5 p-4">
            <div className="eyebrow text-warn mb-1.5">{t("play.downside")}</div>
